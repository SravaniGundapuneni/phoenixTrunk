...
(function($){
    ...
    function saveWidgetFiles(wid, wtype,wname, that){
        //  close the modal
        $('#configModal').foundation('reveal', 'close');
        $(".loadScreenFileSaveOverlay").show();
        //  saving css file
        $.ajax({
                url: _siteroot+"sockets/saveHeaderFooterWidgets.php?widgetId="+wid+"&widgetType="+wtype+"&filetype=css&data="+escape($("#panel2-2 textarea").val()),
                cache: false
        }).done(function (data) {
            var href = _siteroot+"templates/main/core-templates/"+wtype+"/widgets/"+wid+"/css/widget.css";
            ...
            $.ajax({
                ...
                // after saving css files, writing to the phtml file
                $.ajax({
                        url: _siteroot+"sockets/saveHeaderFooterWidgets.php?widgetId="+wid+"&widgetType="+wtype+"&filetype=phtml&data="+escape($("#panel2-1 textarea").val()),
                        cache: false
                }).done(function (data) {
                    $( that ).parent().find(".gm-content .widget").remove().load( _siteroot+"templates/main/core-templates/"+wtype+"/widgets/"+wid+"/widget.phtml" );
                    $.ajax({
                        url : _siteroot+"templates/main/core-templates/"+wtype+"/widgets/"+wid+"/widget.phtml",
                        dataType: "text"
                    });

                    // after saving phtml files, writing to the js file
                    $.ajax({
                            url: _siteroot+"sockets/saveHeaderFooterWidgets.php?widgetId="+wid+"&widgetType="+wtype+"&filetype=js&data="+encodeURIComponent($("#panel2-3 textarea").val()),
                            cache: false,
                    }).done(function (data) {
                        var src = _siteroot+"templates/main/core-templates/"+wtype+"/widgets/"+wid+"/js/widget.js";
                        ...
                        $.ajax({
                            ...
                            success : function (data) {
                                ...
                                var randomWidgetNumber = Math.floor(Math.random() * 999) + 1;
                                var edit_widget = '<a title="Edit Widget" id="edit-widget" data-widgetName="'+wname+'" data-widgetId="'+wid+'" data-widgetType="'+wtype+'" class="edit-widget" ><span class="fa fa-edit"></span></a>';
                                $(that).find(".widget").prepend($(edit_widget));
                                loadWidgetFiles(wid, wtype, false);
                            }
                        })
                    });
    ...
    function loadWidgetFiles(wid, wtype, flag)
    {
            ...
            $.ajax({
                ...
                dataType: "html",
                success : function (data) {
                    $("#panel2-1 textarea").val(data);
                    $.ajax({
                        url : href,
                        dataType: "text",
                        success : function (data) {
                            $("#panel2-2 textarea").val(data);
                        }
                    });
                }
            }).done(function(){
                //  adding the widget js file
                var src = _siteroot+"templates/main/core-templates/"+wtype+"/widgets/"+wid+"/js/widget.js";
                var jsLink = $("<script type='text/javascript' src='"+src+"'>");
                $("head").append(jsLink); 
                ...
                $.ajax({
                    ...
                    dataType: "script",
                    success : function (data) {
                        $("#panel2-3 textarea").val(data);
                    }
                });

            //  checking config file
            var config = _siteroot+"templates/main/core-templates/"+wtype+"/widgets/"+wid+"/config.php?ajaxReturn=true";
            $.ajax({
                url : config,
                dataType: "JSON",
                success : function (d) {
                    $.ajax({
                            url: _siteroot+"sockets/createSiteWidgetsConfig.php?widgetId="+wid+"&widgetType="+wtype+"&data="+JSON.stringify(d),
                            cache: false,
                            success: function(data){
                                if(flag)
                                {
                                    $('#configModal').foundation('reveal', 'open');
                                    flag = false;
                                }
                            },
                            error: function(){
                        ...
                        $("#panel2-4").html(t);
                        $(".loadScreenFileOverlay").hide();
                        $(".loadScreenFileSaveOverlay").hide();
                    });
    ...
    GridElement.prototype.editWidgets = function(that) {
        $('#thisHolderWID').val($(that).attr('data-widgetId'));        
        $('#thisHolderWTYPE').val($(that).attr('data-widgetType'));
        $('#thisHolderWNAME').val($(that).attr('data-widgetName'));
        
        var wid = $(that).attr('data-widgetId');
        var wtype = $(that).attr('data-widgetType');
        var wname = $('#thisHolderWNAME').val();
        $(".loadScreenFileOverlay").show();
        
        loadWidgetFiles(wid,wtype, true);       
        
        $(document).on('click', '#save-config', function(e) {
            console.log("in save");
            saveWidgetFiles(wid,wtype,wname, $(that).parent());
        });
    };
   
    GridElement.prototype.deleteWidgets = function(that) {
        //$('#myModalDeleteWidget').foundation('reveal', 'open');
        $('#thisHolderWID').val($(that).attr('data-widgetId'));
        $('#thisHolderWTYPE').val($(that).attr('data-widgetType'));
        var classId = $(that).attr('data-widgetId');
        $(that).parent().parent().parent().removeClass($(that).attr('data-widgetId'));
        $('#deleteWidget').click();
    };
    
    
    $(document).on('click', '.edit-widget', function(e) {
        foo.editWidgets(this);
        e.preventDefault();
    });
    
    $(document).on('click', '.delete-widget', function(e) {
        foo.deleteWidgets(this);
        e.preventDefault();
    });
    
    $(document).on('click', '#deleteWidget', function(e) {
            $(".loadScreenWidgetDeleteOverlay").show();
            var theWidgetToDelete = $('#thisHolderWID').val();
            var theWidgetTypeToDelete = $('#thisHolderWTYPE').val();

            $.ajax({
                url: _siteroot+"sockets/deleteHeaderFooterWidget.php?widgetId="+theWidgetToDelete+"&widgetType="+theWidgetTypeToDelete,
                cache: false
            }).done(function(data){
                $(document).find(".column").find(".gm-editable-region").each(function(){
                        if($(this).attr("data-widget") == theWidgetToDelete)
                        {
                            $("#panel2-1 textarea").val('');
                            $("#panel2-2 textarea").val('');
                            $("#panel2-3 textarea").val('');
                            $("#panel2-3 textarea").val('');
                            $(this).remove();
                            
                        }
                    });
                    $(".loadScreenWidgetDeleteOverlay").hide();
            });

            $('#myModalDeleteWidget').foundation('reveal', 'close');
    });
    $(document).on('click', '#cancelDeleteWidget', function(e) {
            $('#myModalDeleteWidget').foundation('reveal', 'close');
    });
    
    GridElement.prototype.initDraggableWidgets = function() {
           $( ".draggable" ).draggable({
                helper: 'clone', 
                greedy: true, 
                opacity: 0.70,
                ...
                start: function (e, ui) {
                    ...
                    $(document).find(".gm-editable-region").each(function (){
    ...
    GridElement.prototype.initDroppableWidgets = function() {
        var widgetNameId = '';
        var widgetName = 'test';
        var current;
           $( ".gm-editable-region" ).droppable({
                tolerance: "pointer",
                drop: function( event, ui ) {
                
                var randomWidgetNumber = Math.floor(Math.random() * 999) + 1;
                widgetName = ui.draggable.attr('id');
                var widgetType = ui.draggable.attr("data-widgetType");
                
                if(widgetType == 'content')
                    widgetType = $("#templateName").val().trim();
                
                widgetName = widgetName.replace('draggable-','');
                if(widgetName == 'PropertiesGridWidget')
                    var widgetNameWithHyphens = 'properties-grid-widget';
                else
                    var widgetNameWithHyphens = widgetName;
                
                $(".loadScreenWidgetOverlay").show();
                
                $( this ).attr("data-widget",widgetName+"_"+randomWidgetNumber);
                //$( this ).parent().addClass(widgetName+"_"+randomWidgetNumber);
                $( this ).attr("data-widget-type",widgetType);
                if ( $( ".callout" ).length ) 
                {
                    $( this ).children('.callout').remove();
                }
                
                var tmplName = '';
                var widgetId = randomWidgetNumber;
                widgetNameId = widgetNameWithHyphens+'_'+widgetId;
                
                current = $(this);
                $( this ).find("p").remove();
                $( this ).find(".gm-delete").addClass("delete-widget");
                $( this ).find(".gm-delete").attr("data-widgetName",widgetName);
                $( this ).find(".gm-delete").attr("data-widgetId",widgetNameId);
                $( this ).find(".gm-delete").attr("data-widgetType",widgetType);
                
                //  call to a copy script to load the widget files into the site-widget folders
                $.ajax({
                        type: 'POST',
                        url: addHeaderFooterWidgets,
                        data: "widgetId="+widgetId+"&widgetName="+widgetName+"&widgetType="+widgetType,
                        cache: false,
                }).done(function (data) {
                    
                    //  adding the widget css file
                    var href = _siteroot+"templates/main/core-templates/"+widgetType+"/widgets/"+widgetNameId+"/css/"+widgetNameWithHyphens+".css";
                    var cssLink = $("<link rel='stylesheet' type='text/css' href='"+href+"'>");
                    $("head").append(cssLink); 
                    ...
                    $.ajax({
                        type: 'POST',
                        url : _siteroot+"templates/main/core-templates/"+widgetType+"/widgets/"+widgetNameId+"/"+widgetNameWithHyphens+".phtml",
                        dataType: "html",
                        success : function (data) {
                            $("#panel2-1 textarea").text(data);
                            $( current ).append(data);
                            //$( current ).load(_siteroot+"templates/main/core-templates/"+widgetType+"/widgets/"+widgetNameId+"/"+widgetNameWithHyphens+".phtml");
                            var edit_widget = '<a title="Edit Widget" id="edit-widget"  data-widgetName="'+widgetNameId+'" data-widgetId="'+widgetNameId+'" data-widgetType="'+widgetType+'" class="edit-widget"><span class="fa fa-edit"></span></a>';
                            $(current).find(".widget").prepend($(edit_widget));
                            
                            $.ajax({
                                url : href,
                                dataType: "text",
                                success : function (data) {
                                    $("#panel2-2 textarea").text(data);
                                }
                            });
                        }
                    }).done(function(){
                        //  adding the widget js file
                        var src = _siteroot+"templates/main/core-templates/"+widgetType+"/widgets/"+widgetNameId+"/js/"+widgetNameWithHyphens+".js";
                        var jsLink = $("<script type='text/javascript' src='"+src+"'>");
                        $("head").append(jsLink); 
                        ...
                        $.ajax({
                            ...
                            dataType: "script",
                            success : function (data) {
                                $("#panel2-3 textarea").text(data);
                                $(".loadScreenWidgetOverlay").hide();
                            }
                        });
                    });
                }).fail(function() {
    ...
    GridElement.prototype.initSortable = function(col, gm) {
            $(col).sortable({
...
/*------------------------------------------ Canvas & Controls ---------------------------------------*/
      ...
      /**
        ...
        gm.saveremote =  function(){
            if(!$('#layoutName').attr('value'))
            {
                alert('Please add the layout name!');
                return false;
            }
        var canvas=gm.$el.find("#" + gm.options.canvasId);
            $.ajax({
              ...
              data: {content: canvas.html(),'layoutName':$('#layoutName').attr('value')},
              success: function(){
                  alert("Layout file saved successfully!");
...
/*------------------------------------------ Editable Regions ---------------------------------------*/
        ...
        gm.addEditableAreaClick = function(container, btn) {
                      console.log('clicked editable area!');
          var cTagOpen = '';//'<!--'+gm.options.gmEditRegion+'-->',
              cTagClose = '';//'<!--\/'+gm.options.gmEditRegion+'-->',
              elem = null;
        ...
        gm.initNewContentElem = function(newElem) {
          ...
          $.each(parentCols, function(i, col) {
            ...
            console.log('foo sortable init');
            foo.initSortable(col, gm);
          });
          
          foo.initDraggableWidgets();
          foo.initDroppableWidgets();
        };
