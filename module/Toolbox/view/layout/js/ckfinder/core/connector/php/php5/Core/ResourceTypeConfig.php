...
class CKFinder_Connector_Core_ResourceTypeConfig
{
    ...
    /**
     * Array with allowed folders
     *
     * @var array[]string
     * @access private
     */
    private $_skipFolders = array();
    /**
    ...
    function __construct($resourceTypeNode)
    {
        ...
        if (isset($resourceTypeNode["skipFolders"])) {
            if (is_array($resourceTypeNode["skipFolders"])) {
                foreach ($resourceTypeNode["skipFolders"] as $e) {
                    $this->_skipFolders[] = strtolower(trim((string)$e));
                }
            }
            else {
                $resourceTypeNode["skipFolders"] = trim((string)$resourceTypeNode["skipFolders"]);
                if (strlen($resourceTypeNode["skipFolders"])) {
                    $extensions = explode(",", $resourceTypeNode["skipFolders"]);
                    foreach ($extensions as $e) {
                        $this->_skipFolders[] = strtolower(trim($e));
                    }
                }
            }
        }
     ...
     * @return array[]string
     */
    public function checkSkipFolders($file,$folders)
    {
        return in_array($file, $folders);
    }
