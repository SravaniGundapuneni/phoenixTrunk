...
class CKFinder_Connector_CommandHandler_DbSaveNewFiles extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
	...
	//function onAfterFileUpload( &$command )
	function onAfterFileUpload()
	{
		/*
		$logger = new Logger();
		$logger->addWriter('stream', null, array('stream' => SITE_PATH . '/logs/jason.log'));
		$logger->info('foo');
		*/

		/*
		if ( $command == 'DbSaveNewFiles' )
		{
...
$dbSaveNewFiles = new CKFinder_Connector_CommandHandler_DbSaveNewFiles();
// Register the onBeforeExecuteCommand method to be called by the BeforeExecuteCommand hook.
$config['Hooks']['AfterFileUpload'][] = array($dbSaveNewFiles, "onAfterFileUpload");
