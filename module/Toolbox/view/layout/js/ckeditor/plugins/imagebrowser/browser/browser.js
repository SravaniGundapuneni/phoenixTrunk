...
CkEditorImageBrowser.loadData = function (url, onLoaded) {
	...
	$.getJSON(url, function (list) {
		$.each(list, function (_idx, item) {
			...
			CkEditorImageBrowser.addImage(item.folder, item.image, item.thumb, item.filename);
		});
...
CkEditorImageBrowser.addImage = function (folderName, imageUrl, thumbUrl, fileName) {
...
CkEditorImageBrowser.renderImagesForFolder = function (folderName) {
	...
	$.each(images, function (_idx, imageData) {
		var html = templateHtml;
                ...
                var parts = imageData.fileName.split('.');
                if(parts[parts.length-1] == 'pdf')
                {
                    html = html.replace('%imageUrl%', imageData.imageUrl);
                    html = html.replace('%thumbUrl%', 'images/pdf.png');
                }
                else
                {
                    html = html.replace('%imageUrl%', imageData.imageUrl);
                ...
                html = html.replace('%fileName%', imageData.fileName);
