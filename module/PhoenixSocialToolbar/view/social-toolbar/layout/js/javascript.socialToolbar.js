var socialtoolbar = (function(){
    ...
    // html code replaced with each social media javascript
        ...
        left_buttons[1]='<div class="g-plusone" data-size="medium"></div>'; 
        left_buttons[2]="<a id=\"pinterest-btn\" href=\"#\" onclick=\"javascript:void((function(){var e=document.createElement('script');e.setAttribute('type','text/javascript');e.setAttribute('charset','UTF-8');e.setAttribute('src','http://assets.pinterest.com/js/pinmarklet.js?r='+Math.random()*99999999);document.body.appendChild(e)})());\"><img border=\"0\" src=\""+module_path+"/themes/default/images/PinExt.png\" title=\"Pin It\" /></a>";
      ...
      jQuery(document).ready(function(){     
        ...
        var content = build_content();
                ...
                if (layout==1 && global_settings.toolbarSize){
                    ...
                    toolbarSize = 'style="width:'+size+'px; margin-left: -'+(size/2)+'px;"';
                }
                ...
                if (layout == 1 && global_settings.extend == 1){
                    ...
                    '<!-- facebook -->'+
                    '<div id="fb-root"></div>'+
                    '<div id="socialToolbarContainer" class="stLayout'+layout+' '+borderStyle+'" '+toolbarSize+'>' + 
                            '<div id="socialToolbarBackground"></div>'+
                            '<div id="socialToolbarInner" class="box-shadow inner_toolbar">'+content+'</div>' + // full toolbar
                            '<div id="socialToolbarInnerMin" class="box-shadow inner_toolbar minimized" ><a href="#" onclick="{socialtoolbar.maximize(); return false;}" id="social_toolbar_maximize"><div style="float:left;"><span class="stdButton">'+socialToolbarSettings.minimzedLabelHorizontal+'</span></div> <span class="icon">&nbsp;</span></a></div>' + // minimized version
                    '</div>');
        ...
        //Layout 1 - Default (Horizontal)
                if (layout == 1){
                    ...
                    if (jQuery.cookie("socialToolbarMin")==1) {
                       ...
                       jQuery("#socialToolbarInnerMin").css({bottom:"0px"});
                    }
                }else{
                ...
                /*
                jQuery(".shareRight .contentLeft").mouseleave(function(){
                        jQuery(".shareRight .contentLeft").hide();
                });
                jQuery(".shareLeft .contentLeft").mouseleave(function(){
    ...
    function build_content() {
            ...
            if (layout == 1 && global_settings.toolbar_label ) {
            ...
            //Layout 1 - Default (Horizontal)
            if (layout == 1){
                if (global_settings.share_tooltip == 1)
    ...
    function build_right_content() {
           ...
           // }
                ...
                for (var key in obj) {
                        if (obj.hasOwnProperty(key)) {
                                        ...
                                        if (obj[key].enabled==1)  {
                                                ...
                                                //Layout 1 - Default (Horizontal)
                                                if (layout == 1){
                                                    right_content=right_content+'<div class="socialToolbarButtonRight '+iconPreset+'"><a href="#" onclick="{socialtoolbar.win_init_'+i+'(\''+json_settings+'\',this); return false;}" id=social_toolbar_win_open_'+i+'><span class="stdButton'+onlyIcons+'"  ><b class="'+key.toLowerCase()+'" ></b>'+key+'</span></a></div>'; 
                                                }else{
            ...
            return right_content+'<a href="#" onclick="{socialtoolbar.minimize(); return false;}"  title="Minimize" id="social_toolbar_minimize"><span class="icon">&nbsp;</span></a>';
    }
...
// Define functions responsible for windows content
    ...
    function win_init_1(json_settings,element) {
        ...
        if (obj.enabled==1) {
               ...
               var content = '<iframe id="smo-facebook2" src="//www.facebook.com/plugins/likebox.php?href='+encodeURIComponent(obj.url)+'&amp;width=290&amp;height=320&amp;'+displayType+'&amp;header=false" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:300px; height:320px;" allowTransparency="false"></iframe>';
               win_open(1,'Facebook',content,300, 320,element);
        }
    ...
    function win_init_2(json_settings,element) {
        ...
        if (obj.enabled==1) {
            var content = '<iframe id="smo-twitter" scrolling="no" frameborder="0" width="'+w+'" height="'+h+'" src="'+global_settings.module_path+'/helpers/twitter.php?username='+username+'&tweets_links='+tweets_links+'&width='+w+'&height='+h+'"></iframe> ';             
            win_open(2,'Twitter',content,w, h,element);
        }
    }
    ...
    function win_init_3(json_settings,element) {
        ...
        if (obj.enabled==1) {
            ...
            //var content = '<iframe id="smo-youtube1" class="leftWindow" width="560" height="315" src="'+obj.url+'" frameborder="0" allowfullscreen></iframe>';
                        var content = '<object width="560" height="315"><param name="movie" value="http://www.youtube.com/v/'+video_id+'?version=3&amp;hl=es_ES"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><embed src="http://www.youtube.com/v/'+video_id+'?version=3&amp;hl=es_ES" type="application/x-shockwave-flash" width="560" height="315" allowscriptaccess="always" allowfullscreen="true"></embed></object>';
    ...
    function win_init_4(json_settings,element) {
        ...
        if (obj.enabled==1) {
            ...
            if (matches!=null) {
                ...
                var content = '<iframe id="smo-tripadvisor1" scrolling="no" frameborder="0" width="'+(w-20)+'" height="530px" src="'+global_settings.module_path+'/helpers/tripadvisor.php?id='+tripadvisor_id+'&width='+(w-55)+'&height='+(h-30)+'"></iframe> ';
                 
            } else {
                var content ='Error: Incorrect profile URL';
            }
                       
            win_open(4,'Tripadvisor',content,w, h,element);
    ...
    function mail_win_init( element ) {
        ...
        var content= '<div class="socialCloseButton" ><a onclick="socialtoolbar.win_close();">'+socialToolbarSettings.closeButton+'</a></div>';
                ...
                content +='<iframe id="iframe_mail" scrolling="no" frameborder="0" width="'+w+'" height="'+h+'" src="'+global_settings.module_path+'/helpers/send_email.php?title='+encodeURI(document.title)+'&color='+global_settings.color+'&path='+global_settings.module_path+'&description='+metaDescription+'&&fColor='+fontColor+'&url='+encodeURI(document.URL)+'"></iframe>';
