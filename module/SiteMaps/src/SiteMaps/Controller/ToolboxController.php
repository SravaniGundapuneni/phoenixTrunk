...
class ToolboxController extends \ListModule\Controller\ToolboxController
{
    ...
    public function addItemAction()
    {
        ...
        $service->scanFolder();
        return $this->redirect()->toRoute('siteMaps-toolbox');
    }
    ...
    public function editlistAction()
    {
        ...
        $taskViewModel=$serviceLocator->get('phoenix-sitemaps')->getAllList();
	   ...
	   if ($this->getRequest()->isPost())
		{
			...
			else
			{
				...
				$siteMapService=$this->getServiceLocator()->get('phoenix-sitemaps');
				//$editviewModel = $this->doEditList($siteMapService);
				$sectionService = $this->getServiceLocator()->get('phoenix-sitemapsection');
				$pageSectionService=$this->getServiceLocator()->get('phoenix-sitemappages');
				$dataSection=$sectionService->getDataSection();
				$pageDataSection=$pageSectionService->getPageDataSection();
			
				foreach($dataSection as $sectionId)
				{
					//var_dump($sectionId);
					if($postdata['dataSectionId']==$sectionId->getId())
					{
						
						$insertSecId=$sectionId;
					}
					else
					{
							$intflag=1;
				...
				if($intflag==1)
					{
						...
						$pageKey = $postdata['pageKey']; 
						
						$data=array(
							'dataSectionId'=> $insertSecId,
							'pageKey'=>$pageKey,
							'visible'=>$visible,
				 
							);
						//var_dump($data);
						$siteMapService->createSiteMap($data,true);
					}
				
			    //$dataSectionId=$postdata['dataSectionId'];
			  
					 
				//$result=$urreview->sendEmail($remail,$subject,$name,$data);
				//$this->save($this->createModel(), $data);
				//var_dump($dataSectionId);
				//exit;
